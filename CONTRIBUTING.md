# Contributing to quantisnetcore-lib

We're working hard to make *quantisnetcore-lib* the most powerful JavaScript library for working with Quantsisnet. Our goal is to have *quantisnetcore-lib* be a library that can be used by anyone interested in Quantsisnet, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.quantisnet.org/forum (Support Forum)
...
These are some global design goals in quantisnetcore-lib that any change must adhere.
...
When possible, quantisnetcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of quantisnetcore-lib methods. For example, calling `JSON.stringify` on an quantisnetcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'quantisnetcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the quantisnetcore object.

DO:
```javascript
var quantisnetcore = require('../');
var PublicKey = quantisnetcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add quantisnetevo git@github.com:quantisnetevo/quantisnetcore-lib.git
git pull --rebase quantisnetevo master
```
...
```
Finally go to [github.com/quantisnetevo/quantisnetcore-lib](https://github.com/quantisnetevo/quantisnetcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of quantisnetcore-lib.

If you have any questions feel free to post them to
[github.com/quantisnetevo/quantisnetcore-lib/issues](https://github.com/quantisnetevo/quantisnetcore-lib/issues).
